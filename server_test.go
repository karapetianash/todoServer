...
func TestGet(t *testing.T) {
	testCases := []struct {
		...
		{name: "GetRoot", path: "/", expCode: http.StatusOK,
			expContent: "There's an API here"},
		{name: "GetAll", path: "/todo",
			expCode:  http.StatusOK,
			expItems: 2, expContent: "Task number 1."},
		{name: "GetOne", path: "/todo/1",
			expCode:  http.StatusOK,
			expItems: 1, expContent: "Task number 1."},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case r.Header.Get("Content-Type") == "application/json":
				...
				if resp.Results[0].TaskName != tc.expContent {
					t.Errorf("expected %q, got %q", tc.expContent, resp.Results[0].TaskName)
				}
...
func TestAdd(t *testing.T) {
	...
	t.Run(taskName, func(t *testing.T) {
		...
		if r.StatusCode != http.StatusCreated {
			t.Errorf("Expected %q, got %q.", http.StatusCreated, r.StatusCode)
		}
	})
	...
	t.Run("CheckAdd", func(t *testing.T) {
		...
		if resp.Results[0].TaskName != taskName {
			t.Errorf("Expected %q, got %q.", taskName, resp.Results[0].TaskName)
		}
	})
}
...
func TestDelete(t *testing.T) {
	...
	t.Run("CheckDelete", func(t *testing.T) {
		...
		if resp.Results[0].TaskName != expTask {
			t.Errorf("Expected %q, got %q.", expTask, resp.Results[0].TaskName)
		}
	})
}
